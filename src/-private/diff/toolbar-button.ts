...
type CrepeFeatureConfigs = NonNullable<
  ConstructorParameters<typeof Crepe>[0]
>['featureConfigs'];

const DIFF_ICON =
  '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 3v14M8 17l-4-4M8 17l4-4M16 21V7M16 7l4 4M16 7l-4 4"/></svg>';

function isDiffActive(ctx: Ctx): boolean {
  const view = ctx.get(editorViewCtx);
  return diffPluginKey.getState(view.state)?.active ?? false;
}

function onToggle(ctx: Ctx): void {
...
export function diffFeatureConfigs(): CrepeFeatureConfigs {
  return {
